...
It teaches `SKILL.md`-compatible agents to use the MCP tools or CLI well: search
cheap dates first, set currency and country, retrieve booking URLs, rank results,
and flag fare risks. MCP supplies the tools. The skill supplies the workflow.
Agents without MCP can use the same workflow through the CLI.
...
chmod +x hooks/pre-push  # Unix/macOS only
```
The hook runs `cargo fmt`, `cargo clippy`, `rustdoc`, `cargo test --lib`, and offline pytest
before every **push** (i.e. before you open or update a PR) — not on every commit, so local
commits stay fast. Skip it for a work-in-progress push with `git push --no-verify`. Requires
`maturin develop` to have been run at least once for the Python check.
